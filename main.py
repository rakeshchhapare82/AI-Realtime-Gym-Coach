...
def main():
    ...
    if "voice_pipeline" not in st.session_state:
        try:
            # Load environment from .env (if present) and override existing values
            load_dotenv(override=True)

            api_key = os.environ.get("GROQ_API_KEY", "")

            # Fallback to Streamlit secrets if .env not used
            if not api_key and hasattr(st, "secrets") and "GROQ_API_KEY" in st.secrets:
                api_key = st.secrets["GROQ_API_KEY"]

            if not api_key:
                st.error("GROQ_API_KEY is not set. Please add it to your .env or Streamlit secrets.")
